...
DEFAULT_CHECKOUT_DIR="$(cd "$REPO_ROOT/.." && pwd)/kengine-kotlin-n64"

UPSTREAM_URL="${KENGINE_KOTLIN_UPSTREAM_URL:-https://github.com/JetBrains/kotlin.git}"
CHECKOUT_DIR="${KENGINE_N64_KOTLIN_REPO:-$DEFAULT_CHECKOUT_DIR}"
BRANCH="${KENGINE_N64_KOTLIN_BRANCH:-kengine/n64-mips}"
FORK_URL=""
VERSION=""
TAG=""
YES=0
...
usage() {
    cat <<'EOF'
Usage: setup-n64-kotlin-fork.sh [options]

Clones JetBrains/kotlin at this repo's pinned Kotlin version and creates a
local N64 target branch for Kotlin/Native compiler work.

This is a SEPARATE fork from the Nintendo Switch Kotlin fork. The N64 target
requires MIPS backend support, which is independent of the Switch ARM64 work.

Options:
  --checkout-dir PATH      Kotlin source checkout. Defaults to ../kengine-kotlin-n64.
  ...
  --branch NAME            Local branch to create/use. Defaults to kengine/n64-mips.
...
write_local_properties() {
    ...
    if [ -f "$props_file" ]; then
        existing_contents="$(cat "$props_file")"
    fi

    {
        ...
        printf '# N64 Kotlin fork (generated by setup-n64-kotlin-fork.sh)\n'
        printf 'kengine.n64.kotlinRepo=%s\n' "$CHECKOUT_DIR"
        if [ -x "$native_home/bin/kotlinc-native" ]; then
            printf 'kengine.n64.kotlinNativeHome=%s\n' "$native_home"
        else
            printf '# Build first, then uncomment:\n'
            printf '# kengine.n64.kotlinNativeHome=%s\n' "$native_home"
        fi
        printf '# Use this after the compiler fork knows about the target:\n'
        printf '# kengine.n64.kotlinTarget=n64_mips32\n'
    } > "$props_file"
...
log ""
log "N64 Kotlin fork checkout is ready."
log ""
log "Next steps:"
log "  1. Add MIPS R4300i (VR4300) target definition to kotlin-native/konan/konan.properties"
log "  2. Add n64_mips32 target to the Kotlin/Native compiler"
log "  3. Build the distribution: ./kengine-kotlin/build-kotlin-native-dist.sh --checkout-dir $CHECKOUT_DIR"
log "  4. Configure kengine-kotlin/local.properties with kengine.n64.kotlinNativeHome"
