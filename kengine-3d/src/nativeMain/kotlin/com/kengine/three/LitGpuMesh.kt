    ...
    companion object {
        fun create(
            ...
            require(vertices.isNotEmpty()) {
                "LitGpuMesh requires at least one vertex."
            }

            val values = FloatArray(vertices.size * LitVertex3D.FLOATS_PER_VERTEX)
            vertices.forEachIndexed { index, vertex ->
                vertex.writeTo(values, index * LitVertex3D.FLOATS_PER_VERTEX)
            }
        ...
        private fun create(
            ...
            return memScoped {
                ...
                val vertexBuffer = SDL_CreateGPUBuffer(gpu.device, bufferInfo.ptr)
                    ?: throw IllegalStateException("Error creating lit GPU vertex buffer: ${SDL_GetError()?.toKString()}")
                ...
                LitGpuMesh(gpu, vertexBuffer, vertexCount)
            }
        }
        ...
        private fun createTransferBuffer(
            ...
            return memScoped {
                ...
                SDL_CreateGPUTransferBuffer(gpu.device, transferInfo.ptr)
                    ?: throw IllegalStateException("Error creating lit GPU transfer buffer: ${SDL_GetError()?.toKString()}")
            }
        }
        ...
        private fun uploadVertexData(
            ...
            val mapped = SDL_MapGPUTransferBuffer(gpu.device, transferBuffer, false)
                ?: throw IllegalStateException("Error mapping lit GPU transfer buffer: ${SDL_GetError()?.toKString()}")
            ...
            memScoped {
                val commandBuffer = SDL_AcquireGPUCommandBuffer(gpu.device)
                    ?: throw IllegalStateException("Error acquiring lit GPU command buffer: ${SDL_GetError()?.toKString()}")
                val copyPass = SDL_BeginGPUCopyPass(commandBuffer)
                    ?: throw IllegalStateException("Error beginning lit GPU copy pass: ${SDL_GetError()?.toKString()}")
                ...
                if (!SDL_SubmitGPUCommandBuffer(commandBuffer)) {
                    throw IllegalStateException("Error submitting lit GPU upload command buffer: ${SDL_GetError()?.toKString()}")
                }
            }
        }
    }
