...
- **Playdate**: Blocked — Kotlin/Native linuxArm32Hfp emits ARMv4, Playdate requires ARMv7E-M (Cortex-M7). Opt-in via `-Pkengine.enablePlaydate=true`.
- **Nintendo Switch**: Experimental opt-in backend via `-Pkengine.enableNintendoSwitch=true`; uses a libnx C host plus Kotlin/Native static library and portable 2D input/audio/render command buffers. Current focus is 2D runtime coverage and save data; networking and 3D are deferred.
