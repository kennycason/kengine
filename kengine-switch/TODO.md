...
- Kotlin-linked Switch build compiles and packages as `build/switch/kengine-switch.nro`.
...
- Latest run still crashes, but it moved:
  ...
  - Latest PC: `kengine-switch:0x1c7f8`.
...
1. Trace every remaining `tpidr_el0` instruction in `build/switch/kengine-switch.elf`.
...
7. Re-run `build/switch/kengine-switch.nro` in Ryujinx.

## Useful Commands

```bash
jenv exec ./kengine-kotlin/build-kotlin-native-dist.sh --task :kotlin-native:distCompiler --no-local-properties
jenv exec ./gradlew -Pkengine.switch=true :kengine-switch:clean :kengine-switch:buildSwitchNro
...
```bash
/opt/devkitpro/devkitA64/bin/aarch64-none-elf-objdump -d kengine-switch/build/switch/kengine-switch.elf | rg -n -C 4 "tpidr|_konan_function_0_impl|ScopedRunnableState|ThreadSuspensionData"
/opt/devkitpro/devkitA64/bin/aarch64-none-elf-addr2line -e kengine-switch/build/switch/kengine-switch.elf -f -C 0x1c7f8 0x26b50 0x26a64 0x0914
```
...
perl -pe 's/\0//g' ~/Library/Logs/Ryujinx/Ryujinx_*.log | rg -n "Invalid memory|Guest stack trace|kengine-switch|PC:|X\\["
```
