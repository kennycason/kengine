...
- `kengine-3d-ui` provides the first GPU-compatible UI layer for `SDL_GPU_3D` windows: retained row/column views, labels, buttons, sliders, mouse hit testing, drag focus, solid rectangles, and SDL_ttf-backed text textures rendered through screen-space GPU quads.
...
- `kengine-3d-importer` is a top-level 3D tooling module that keeps runtime loaders honest: GLB/GLTF/OBJ are identified as runtime-ready and validated through `ModelLoader3D.inspect`, FBX/USD-family inputs are reported as requiring external GLB export, and unsupported source formats receive actionable messages without pulling converter dependencies into games or runtime rendering.
...
The current runtime-loading formats are GLB, GLTF, and Wavefront OBJ. Games should load those through `ModelLoader3D` / `AnimatedModelLoader3D` and keep source-format export outside Kengine.
...
For source formats such as FBX and USDZ, require external export into GLB from the user's asset tool. `kengine-3d-importer` owns the preflight boundary: it can identify source formats, suggest a runtime GLB path, and validate supported runtime assets through `ModelLoader3D.inspect`, but it does not invoke Blender, Assimp, the FBX SDK, or USD tooling.

Avoid putting Assimp, Blender scripting, or USD tooling into `kengine-3d` runtime loading. Those dependencies should stay out of Kengine unless we have a concrete game-runtime reason to ship them.
...
- Done: Add model preset cycling, animation clip/playback controls, background presets, lighting presets/strength controls, and debug light/axis visualization to `kengine-3d-model-viewer`.
...
- Done: Add `kengine-3d-importer` as the top-level preflight boundary for runtime-ready formats versus source formats that require external GLB export.
...
- How much of the existing Kengine UI API should `kengine-3d-ui` mirror versus keeping a smaller 3D-tooling-specific surface?
- Which widgets should become shared cross-backend interfaces once both SDL renderer UI and GPU UI have enough overlap?
