...
- `kengine-3d-model-viewer` is a top-level 3D tooling app that opens an SDL GPU window, loads static or animated model assets through the reusable source/upload path, previews animated clips, and provides orbit camera, model preset, clip, playback, background, and lighting controls outside a game demo.
...
- Next: Add a GPU-compatible inspector surface to `kengine-3d-model-viewer` for clickable model metadata, animation clip selection, material/texture diagnostics, and camera controls.
