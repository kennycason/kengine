...
Status: GPU window, primitive rendering, depth proof, mesh rendering, Rubik's cube, and first texture/material path started
...
- `kengine-3d` provides the first mesh path: `GpuMesh`, `CubeFaceColors`, `Vertex3D`, `MeshRenderer3D`, `Mat4`, `Transform3D`, and `PerspectiveCamera`.
- `kengine-3d` provides the first texture/material path: `GpuTexture`, `TextureVertex3D`, `TexturedGpuMesh`, and `TexturedMeshRenderer3D`.
- `games:kengine-3d-demos` opens a GPU-backed window and renders multiple primitives, a rotating color cube, and a rotating textured cube with depth testing enabled.
...
Current shader note: `PrimitiveRenderer3D`, `MeshRenderer3D`, and `TexturedMeshRenderer3D` use inline MSL shader source for the macOS proof. Before treating the renderer as cross-platform, add a repeatable shader build/package path for MSL, SPIR-V, and DXIL.
...
### Asset Loading: glTF 2.0 First

Start with glTF 2.0 as the only 3D asset format.
