...
Status: GPU window, primitive rendering, depth proof, mesh rendering, and Rubik's cube demo started
...
- `games:kengine-3d-demos` opens a GPU-backed window and renders multiple primitives plus a rotating cube mesh with depth testing enabled.
...
Current shader note: `PrimitiveRenderer3D` and `MeshRenderer3D` use inline MSL shader source for the macOS proof. Before treating the renderer as cross-platform, add a repeatable shader build/package path for MSL, SPIR-V, and DXIL.
...
- Load image pixels.
- Upload to `SDL_GPUTexture`.
- Add sampler support.
- Add simple unlit textured material.

Success criteria:

- Textured cube or plane.
